...
class _FoldableDirectoryTreeState extends State<FoldableDirectoryTree> {
    ...
    return Column(
      ...
      children: [
        GestureDetector(
          ...
          onLongPress: () {
            _showBottomSheet(context, directory, stateNotifier);
          },
          child: MouseRegion(
              ...
              children: [
                ...entries.map((entry) {
                  if (entry is Directory) {
                    ...
                    return _buildFileItem(entry, stateNotifier);
                  }
  ...
  Widget _buildFileItem(File file, DirectoryTreeStateNotifier stateNotifier) {
    ...
    return GestureDetector(
      ...
      onLongPress: () {
        _showBottomSheet(context, file, stateNotifier);
      },
      child: MouseRegion(
        cursor: SystemMouseCursors.click,
        child: Row(
  ...
  void _showBottomSheet(BuildContext context, FileSystemEntity entity,
      DirectoryTreeStateNotifier stateNotifier) {
          ...
          children: [
            if (entity is Directory) ...[
              if (widget.enableCreateFileOption)
                ListTile(
                  leading: widget.folderStyle?.iconForCreateFile ??
                      FolderStyle().iconForCreateFile,
                  title: const Text('Create File'),
                  onTap: () {
                    Navigator.pop(context);
                    stateNotifier.startCreating(entity.path, false);
                  },
                ),
              if (widget.enableCreateFolderOption)
                ListTile(
                  leading: widget.folderStyle?.iconForCreateFolder ??
                      FolderStyle().iconForCreateFolder,
                  title: const Text('Create Folder'),
                  onTap: () {
                    Navigator.pop(context);
                    stateNotifier.startCreating(entity.path, true);
                  },
                ),
            ...
            if (entity is File) ...[
              if (widget.enableDeleteFileOption)
                ListTile(
                  leading: widget.fileStyle?.iconForDeleteFile ??
